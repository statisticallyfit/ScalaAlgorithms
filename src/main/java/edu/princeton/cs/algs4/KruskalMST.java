...
public class KruskalMST {
    ...
    private Queue<Edge> mstEdges = new Queue<Edge>();  // edges in MST
    ...
    public KruskalMST(EdgeWeightedGraph edgeWeightedGraph) {
        ...
        MinPQ<Edge> allEdges = new MinPQ<Edge>();
        ...
        while (!allEdges.isEmpty() && mstEdges.size() < edgeWeightedGraph.V() - 1) {
            Edge e = allEdges.delMin();
            int v = e.either(), w = e.other(v);
            if (!uf.connected(v, w)) { // if v-w does not create a cycle, then...
                uf.union(v, w);  // merge v and w components
                mstEdges.enqueue(e);  // add edge e to mstEdges
                weight += e.weight();
            }
        }
