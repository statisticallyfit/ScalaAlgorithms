...
public class DegreesOfSeparation {
    ...
    public static Queue<String> getShortestPath(SymbolGraph sg, BreadthFirstPaths bfs, String destination) {
        Queue<String> path = new Queue<String>();

        if (sg.contains(destination)) {
            int t = sg.index(destination);
            if (bfs.hasPathTo(t)) {
                for (int v : bfs.pathTo(t)) {
                    path.enqueue(sg.name(v));
                }
                return path;
            }
            else {
                return null; //StdOut.println("Not connected");
            }
        }
     ...
     *
     *  // note answers the question: how do I get from "source" to "input" in the shortest possible way?
     *  example: source = Bacon, Kevin, input = Kidman, Nicole
     *  answer: Kidman, Nicole
                     Bacon, Kevin
                     Woodsman, The (2004)
                     Grier, David Alan
                     Bewitched (2005)
                     Kidman, Nicole
     * example: source = Bacon, Kevin, input = movie Eagle Has Landed
     * answer: Eagle Has Landed, The (1976)
                     Bacon, Kevin
                     JFK (1991)
                     Sutherland, Donald (I)
    ...
    public static void main(String[] args) {
        ...
        // note doesn't seem to really be needed because it already happens in getShortestPath method.
        ...
        while (!StdIn.isEmpty()) {
            ...
            Queue<String> paths = getShortestPath(sg, bfs, destination); // previously named "sink"
            if (paths != null) {
                for (String v : paths) {
                    StdOut.println("   " + v);
                }
                ...
                StdOut.println("   Not in database or lacked connection somewhere.");
            }
        }
    }
}
