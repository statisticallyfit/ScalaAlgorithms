...
public class SymbolGraph {
    // note st and keys structures are inverses of the other.
    private ST<String, Integer> st;  // string -> index // note like JFK -> 0, MCO -> 1, DNE -> 5...
    private String[] keys;           // index  -> string // note like [0] JFK, [1] MCO, [5] DNE...
    ...
    public SymbolGraph(String filename, String delimiter) {
        ...
        // line to all others
        symbolGraph = new Graph(st.size());
        in = new In(filename);
        while (in.hasNextLine()) {
            String[] a = in.readLine().split(delimiter);
            int v = st.get(a[0]); // note get value for given key name.
            for (int i = 1; i < a.length; i++) {
                int w = st.get(a[i]); // note continue getting values of given key names.
                symbolGraph.addEdge(v, w); // adding the edge to graph.
            }
        }
    }
    ...
    public ST<String, Integer> getSymbolTable() {return st; }
    ...
    /** // note gets the value of a key: So for "ATL" -> 4 the result would be (4) because it is value of key "ATL"
    ...
    /**
     * note made by me. Prints elements in the symGraph.adj(s) list
     */
    public Stack<String> nearestNeighbors (String source) {
        Stack<String> neighbors = new Stack<String>();

        if (st.contains(source)) {
            int s = this.index(source);
            for (int v : symbolGraph.adj(s)) {
                neighbors.push(this.name(v));
            }
            return neighbors;
        }
        return null;
    ...
    public static void main(String[] args) {
        ...
        SymbolGraph sg = new SymbolGraph(filename, delimiter); // give cmd input as: filenamestuffhere " "
        //Graph actualSymbolGraph = sg.G();

        //System.out.println(actualSymbolGraph);

        while (StdIn.hasNextLine()) {
            String source = StdIn.readLine();

            Stack<String> neighbors = sg.nearestNeighbors(source);
            if (neighbors != null) {
