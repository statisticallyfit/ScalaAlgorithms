...
public class DijkstraAllPairsSP {
    ...
    public DijkstraAllPairsSP(EdgeWeightedDigraph edgeWeightedDigraph) {
        all  = new DijkstraSP[edgeWeightedDigraph.V()];
        for (int v = 0; v < edgeWeightedDigraph.V(); v++)
            all[v] = new DijkstraSP(edgeWeightedDigraph, v);
    }
    ...
    public static void main(String[] args) {
        ...
        DijkstraAllPairsSP asp = new DijkstraAllPairsSP(edgeWeightedDigraph);

        for(int v = 0; v < edgeWeightedDigraph.V(); v++){
            for (int w = 0; w < edgeWeightedDigraph.V(); w++){
                if (asp.hasPath(v, w)){
                    StdOut.printf("%d to %d (%.2f)  ", v, w, asp.dist(v, w));
                    for (DirectedEdge e : asp.path(v, w)) {
                        StdOut.print(e + "   ");
                    }
                    StdOut.println();
                }
