...
public class EdgeWeightedDigraph {
    ...
    public EdgeWeightedDigraph(Iterable<DirectedEdge> edges) {
        Queue<DirectedEdge> queueEdges = (Queue<DirectedEdge>) edges;
        this.E = ((Queue<DirectedEdge>) edges).size();
    ...
    private int getVertexCount(Iterable<DirectedEdge> edges) {
        Queue<DirectedEdge> queueEdges = (Queue<DirectedEdge>) edges;
     ...
     *
     * @param  edgeWeightedDigraph the edge-weighted digraph to copy
     */
    public EdgeWeightedDigraph(EdgeWeightedDigraph edgeWeightedDigraph) {
        ...
        for (int v = 0; v < edgeWeightedDigraph.V(); v++)
            this.indegree[v] = edgeWeightedDigraph.indegree(v);
        for (int v = 0; v < edgeWeightedDigraph.V(); v++) {
    ...
    public Digraph convertToDigraph() {
        ...
        for (Bag<DirectedEdge> bag : adj) {
            for (DirectedEdge e : bag) {
                int v = e.from();
                int w = e.to();
                if (!digraph.getAdj(v).contains(w)){
                    digraph.addEdge(v, w);
                }
            }
        }
    ...
    public ArrayList<DirectedEdge> getAdj(int v) {
        validateVertex(v);
        ArrayList<DirectedEdge> newAdj = new ArrayList<DirectedEdge>();
