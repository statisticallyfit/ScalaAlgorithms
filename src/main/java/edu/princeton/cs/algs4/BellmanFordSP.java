...
public class BellmanFordSP {
    ...
    private int costCount;                      // number of calls to relax()
    private Iterable<DirectedEdge> cycle;  // negative cycle (or null if no such cycle)

    /**
     * Computes a shortest paths tree from <tt>s</tt> to every other vertex in
     * the edge-weighted digraph <tt>G</tt>.
     * @param edgeWeightedDigraph the acyclic digraph
     * @param s the source vertex
     * @throws IllegalArgumentException unless 0 &le; <tt>s</tt> &le; <tt>V</tt> - 1
     */
    public BellmanFordSP(EdgeWeightedDigraph edgeWeightedDigraph, int s) {
        distTo  = new double[edgeWeightedDigraph.V()];
        edgeTo  = new DirectedEdge[edgeWeightedDigraph.V()];
        onQueue = new boolean[edgeWeightedDigraph.V()];
    ...
    private void relax(EdgeWeightedDigraph edgeWeightedDigraph, int v) {
        ...
        for (DirectedEdge e : edgeWeightedDigraph.adj(v)) {
            ...
            //checking for negative cycle every Vth call to relax
            if (costCount % edgeWeightedDigraph.V() == 0) {
                findNegativeCycle();
    ...
    public static void main(String[] args) {
        ...
        BellmanFordSP sp = new BellmanFordSP(edgeWeightedDigraph, s);

        // print negative cycle
        if (sp.hasNegativeCycle()) {
            double totalWeight = 0;
            StdOut.println("Negative cycle found:    ");
            for (DirectedEdge e : sp.negativeCycle()){
                StdOut.println(e);
                totalWeight += e.weight();
            }
            StdOut.println("\nTotal weight = " + Math.round(totalWeight * 100) * 1.0/100);
        }
        ...
        // print shortest paths
            ...
            for (int v = 0; v < edgeWeightedDigraph.V(); v++) {
                if (sp.hasPathTo(v)) {
                    StdOut.printf("%d to %d (%1.2f)  ", s, v, sp.distTo(v));
