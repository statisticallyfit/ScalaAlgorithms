...
public class DijkstraUndirectedSP {
    ...
    private IndexMinPQ<Double> candidatesForRelax;    // priority queue of vertices

    /**
     * Computes a shortest-paths tree from the source vertex <tt>s</tt> to every
     * other vertex in the edge-weighted graph <tt>G</tt>.
     *
     * @param  edgeWeightedGraph the edge-weighted digraph
     * @param  s the source vertex
     * @throws IllegalArgumentException if an edge weight is negative
     * @throws IllegalArgumentException unless 0 &le; <tt>s</tt> &le; <tt>V</tt> - 1
     */
    public DijkstraUndirectedSP(EdgeWeightedGraph edgeWeightedGraph, int s) {
        ...
        candidatesForRelax = new IndexMinPQ<Double>(edgeWeightedGraph.V());
        candidatesForRelax.insert(s, distTo[s]);
        while (!candidatesForRelax.isEmpty()) {
            int v = candidatesForRelax.delMin();
            for (Edge e : edgeWeightedGraph.adj(v))
                relax(e, v);
        }

        // check optimality conditions
        assert check(edgeWeightedGraph, s);
    }

    // relax edge e and update candidatesForRelax if changed
    private void relax(Edge e, int v) {
        int w = e.other(v);
        if (distTo[w] > distTo[v] + e.weight()) {
            distTo[w] = distTo[v] + e.weight();
            edgeTo[w] = e;
            if (candidatesForRelax.contains(w)) candidatesForRelax.decreaseKey(w, distTo[w]);
            else                candidatesForRelax.insert(w, distTo[w]);
        }
    }
    ...
    private boolean check(EdgeWeightedGraph edgeWeightedGraph, int s) {
        ...
        for (int w = 0; w < edgeWeightedGraph.V(); w++) {
    ...
    public static void main(String[] args) {
        ...
        DijkstraUndirectedSP sp = new DijkstraUndirectedSP(edgeWeightedGraph, s);
